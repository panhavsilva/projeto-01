...
public class MateriaController {
    ...
    @GetMapping
    public ResponseEntity<List<MateriaResponse>> get(){
        log.info("GET /materias - solicitação recebida para buscar todos as matérias.");
        return ResponseEntity.ok().body(service.buscarTodos());
    }

    @GetMapping("{id}")
    public ResponseEntity<MateriaResponse> getPorId(@PathVariable Long id){
        log.info("GET /materias/:id - solicitação recebida para buscar matéria com id {}.", id);
        return ResponseEntity.ok().body(service.buscarPorId(id));
    }

    @PostMapping
    public ResponseEntity<MateriaResponse> post(@RequestBody MateriaRequest materiaRequest) throws Exception {
